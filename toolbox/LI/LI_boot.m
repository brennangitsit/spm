...
% ===== Allow for adapting defaults - or for skipping ===== 
  ...
  if use_def == 0
	% Allow for input of lower threshold (eg, to only explore "significant" voxels) 
	  ilt = spm_input('Use lower threshold ?','+1','b',' Yes| No',[1 0],0);
	...
	% NOTE: all constrasts should be either normalized or not, don't mix here!
	  ...
	  if size(check,1) ~= 91 || size(check,2) ~= 109 || size(check,3) ~= 91 || sum(check_dims) == 0
		...
		% ok, not in template space or not in our resolution; check once if all are normalized
		  if ni == 2
			if ninputs > 1
				ask = 'Are all images normalized?';
			else
				ask = 'Is this image normalized?';
			end;
			ni = spm_input(ask,'+1','b',[' Yes| No'],[1 0],1);
		  end;
		...
		% also need to check if masks need to be normalized (must be in-register with functionals)
		  if pre == 1
			...
			if ni == 0
				  ...
				  LI_mreslice(fullfile(stdpth,'randbrain.img'),'LI_complete_mask.img',1,0.1,mask_c);
			end;
		...
		% also create various storage spaces 
		  ...
		  for j = 1:iter;
			...
			% make sure maximum value > threshold
			  if mx <= low
				fprintf(['\n' '\t' 'Warning: lower threshold (' num2str(low) ') exceeds or equals maximum voxel value (' num2str(mx) ') - aborting !' '\n' '\n']);
				return
			  end;
...
% ==========================================================================================================
		...
		% set position and size
		  if li_error == 0
			  ...
			  try
                  ...
                  % fallback ...
                    if isempty(pos) || any(pos(3:4)<200)
                        ...
                        pos = [pos(1)*temp(3) pos(2)*temp(4) round(pos(3)*temp(3)) round(pos(4)*temp(4))];
                        if any(pos(3:4)<200),  pos = [500 20 550 800];  end;
			...
			% put results into one figure?
			  if short == 1


				% seems so
				  set(gcf1, 'Name', 'Bootstrap results (p1/1)');
				...
				% now plot histogram of all bootstrapped li's
				  ...
				  set([t7, t8, t9],'color','b','FontSize',8,'BackgroundColor','w');
				  set(t9,'FontWeight','bold');
				...
				% plot histogram of z-score distribution for all thresholds
				  ...
				  title(['Z-Score histograms (LEFT)'], 'interpreter', 'none', 'Fontweight', 'bold');
				  gca1 = gca;
				  axis([-5 5 0 max(get(gca1, 'Ylim'))]);
				  grid on;
				  xlabel('Z-Scores', 'Fontangle', 'italic', 'Color','k');
				  ...
				  title(['Z-Score histograms (RIGHT)'], 'interpreter', 'none', 'Fontweight', 'bold');
				  gca2 = gca;
				  axis([-5 5 0 max([get(gca1, 'Ylim') get(gca2, 'Ylim')])]);
				  set(gca1, 'Ylim', [0 max([get(gca1, 'Ylim') get(gca2, 'Ylim')])]);
				  grid on;
				  xlabel('Z-Scores', 'Fontangle', 'italic', 'Color','k');
			...
			% try to retain formatting as on screen
			  set(gcf1, 'Units','centimeters')
			  pos = get(gcf1, 'Position');
			  set(gcf1, 'Position',pos, 'PaperSize',[pos(3) pos(4)], 'PaperPositionMode','manual', 'PaperPosition', [pos(1) pos(2) pos(3)*1.2 pos(4)*1.2], 'InvertHardcopy', 'off');
			...
			% with larger collections, LI_boot.ps becomes prohibitively huge (>2GB), so be careful
			  try
				...
				if short == 0
					...
					% try to retain formatting as on screen
					  set(gcf2, 'Units','centimeters')
					  pos2 = get(gcf2, 'Position');
					  set(gcf2, 'Position',pos2, 'PaperSize',[pos(3) pos(4)], 'PaperPositionMode','manual', 'PaperPosition', [pos(1) pos(2) pos2(3)*1.2 pos2(4)*1.2], 'InvertHardcopy', 'off');
			  ...
			  % print(gcf1, '-dpng', '-noui', '-r300', ['LI_boot_s' num2str(i) '_m' num2str(ii) '_p1.png']); % re-enable if you also want output to png-file
			  % print(gcf2, '-dpng', '-noui', '-r300', ['LI_boot_s' num2str(i) '_m' num2str(ii) '_p2.png']); % re-enable if you also want output to png-file


			% ... and delete if no graphics was chosen
			  if graphs == 0
				close(gcf1);
				if short == 0,  close(gcf2);  end;
			  end;
		  end;
	  end;
  end;
...
%  ===== Allow for clean up ===== 
  if script == 0
	...
	% delete files by default; re-enable to ask when called interactively
