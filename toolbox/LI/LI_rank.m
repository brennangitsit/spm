...
% get version; abort if minimum version not found
  ...
  if str2double(ver(regexp(ver,'[0-9]'))) < 8
	...
	uiwait(msgbox(['Sorry, you seem to be using an older version of spm (' ver '); this toolbox expects spm8 or later. We apologize for the inconvenience...'], 'Warning'));
	return;

  else

	  fprintf(['\n' '   Welcome to the LI-toolbox...' '\n' '\n']);
  end;
...
% now define thresholds for imcalc: give extra weight if above hi2 (centile distance to hi2)
  for i=1:iter
	...
	elseif i < (iter-hi2)
		...
		TEMP((OP>t(i-1) & OP<t(i))) = (i-1);

	elseif i > (iter-hi2)

		j = abs(hi2-(iter-i))+1;
		TEMP(OP>t(i-1) & OP<t(i)) = ((i-1)*j);
...
% write output file
  % [p nm e v] = spm_fileparts(a);
  V.fname = fullfile(pwd,'LI_rank.img');
