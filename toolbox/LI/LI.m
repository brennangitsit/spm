...
% 
% Call with li(1) to suppress file-output; passing three input
...
%
% The toolbox was developed under and for spm, versions 8 or higher;
% it expects to reside in spm_dir/toolbox/LI and accesses the 
% /data directory with masks therein; if you unzip the toolbox to
% this directory, it should also be integrated into spm's batch editor;
% if you need to install it elsewhere, change the stdpth variable in LI.m
%
% I am most indebted to those of you who really know how to write code
% (mainly John Ashburner, Volkmar Glauche, Tom Nichols and many others),
% so I could steal it. For disclaimer, license and version information,
% please see the accompanying Readme.txt.
%
% Yours, Marko Wilke
...
% avoid display of Divide by Zero (only necessary for older Matlab versions)
...
% set name for output file; enable alternative below if you want to include the date
  outfile = 'li.txt';
...
% define whether cluster size is used as a killer criterion
  cluskill = 1;
...
% ask how the output is supposed to be; defined via command line; could be re-enabled below
  if nargin > 0
	...
	% to routinely write to file, set out to 2, else, set to 1
	  out = 2;
      
	% to routinely ask, re-enable here
...
% =====  check how images should be thresholded  ===== 
  ...
  elseif thr1 == -5
	...
	% if bootstrapping is chosen, prepare passing to external function
	  meth = '(bootstrap)';
	  fprintf(['     Applying the bootstrap approach.' '\n']);	
	  vc = 0;
	  cm = 'total voxel values'; 
  end;
...
% ===== may choose mask for tissue to include, suggest lobes etc ===== 
  ...
  if script == 0
	...
	if isempty(strfind(B(1,:), 'none'))

		fprintf(['     Thank you for selecting ' num2str(size(B,1)) ' inclusive mask(s).' '\n']);
	else

		fprintf(['     Thank you for selecting no inclusive mask(s).' '\n']);
	end;
  end;
...
% now optionally choose mask for tissue to exclude, suggest midline
  ...
  if ischar(C1)
	...
	elseif C1 == 3
		C = 'none';
	elseif C1 == 4
		C = spm_select([0 1],'image','EXclusive Mask - Done for None',[],stdpth,'*_ex.img');
		if isempty(C)
			C = 'none';
		end;
	end;
  ...
  if script == 0
	...
	if isempty(strfind(C(1,:), 'none'))

		fprintf(['     Thank you for selecting ' num2str(size(C,1)) ' exclusive mask(s).' '\n']);
	else

		fprintf(['     Thank you for selecting no exclusive mask(s).' '\n']);
	end;
  end;
...
% don't offer for no threshold, ranking, smoothing or with iterations (use vv by default)
  if script == 0
	if vc ~=0 && sm == 0
		vc = spm_input('Use voxel count or voxel value?','+1','b',['  Count|* Value'],[1 0],0);
	else
		vc = 0;
	end;
  else
...
% ==========================================================================================================
		...
		% for the screen (out=2), do both; else screen-only
		...
		% ===== convert stuff for display =====
		  ...
		  if ~isempty(strfind(file_vol_ns.descrip,'contrast'))
...
% ===== clean up files; leave graphic open although this may lead to an error if clicked upon  =====
  if script == 0
  ...
  if RA == 1
	...
	fprintf(['\n' '   Temporary files (LI_...[hdr|img|mat]) were -not- deleted' '\n' '\n']);
  end;
