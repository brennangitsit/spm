...
% are passed, then the chosen similarity metric can be compared to the
% contents of these matrices using the appropriate results transformation
% functions. These matrices can be used for classical representational
% similarity analysis or pattern component modeling. If
% cfg.files.components.get_inv = 1, then additionally the field
...
n_chunks = unique(cfg.files.chunk);
if n_chunks == 1
    error('Only one chunk provided. Use make_design_rsa instead of make_design_rsa_cv.')
end
...
if isfield(cfg,'files') && isfield(cfg.files,'label')
    ...
    cfg.files.label = ones(szlabel,n_chunks);
    cfg.files.label(floor(szlabel/2):end,:) = -1;
end

% fill all other fields using make_design_cv
design = make_design_cv(cfg);
design.function.name = mfilename;
design.function.ver = 'v20150420';
...
isupper = isequal(cfg.files.components.index,triu(cfg.files.components.index)); % when there are no in the lower, it is an upper matrix
islower = isequal(cfg.files.components.index,tril(cfg.files.components.index)); % when there are no in the upper, it is an lower matrix
